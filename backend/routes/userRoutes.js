...
router.put("/me", authMiddleware, async (req, res) => {
  ...
  const { username, email, password, oldPassword, enable3d, notifications } =
    req.body;
    ...
    if (password) {
      if (!oldPassword) {
        return res
          .status(400)
          .json({
            message: "A senha antiga é necessária para definir uma nova.",
          });
      }
...
router.delete("/me", authMiddleware, async (req, res) => {
  try {
    ...
    // Remove histórico, favoritos, etc. se necessário
    await prisma.activityHistory.deleteMany({ where: { userId } });
